      ...
      <div className="max-w-4xl mx-auto flex flex-col md:flex-row gap-12">
        <div className="md:w-1/2">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              <button type="submit" className="w-full inline-flex justify-center py-3 px-6 border border-transparent shadow-sm text-base font-medium rounded-md text-white bg-brand-orange-600 hover:bg-brand-orange-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-brand-orange-500 transition-colors">
                Send Message
              </button>
            </div>
          </form>
        ...
        <div className="md:w-1/2">
          ...
          <div className="flex space-x-6">
            {socialLinks.map(link => (
              <a key={link.name} href={link.url} target="_blank" rel="noopener noreferrer" aria-label={link.name} className="text-gray-500 hover:text-brand-orange-600 transition-all duration-300 transform hover:scale-125 hover:-translate-y-1">
